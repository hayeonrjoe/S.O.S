document.addEventListener('DOMContentLoaded', function () {
    ...
    if (loadAction === '/online-complaint/admin/c/load-latest') {
        fetchLatestResults()
            .then(() => {
    ...
    function fetchLatestResults() {
        return fetch('/online-complaint/admin/c/latest-results')
    ...
    function attachRowClickListener() {
        ...
        Array.from(rows).forEach(function(row) {
            row.addEventListener("click", function() {
                ...
                window.location.href = "/online-complaint-comment-form/admin/c?num=" + ocId;
            });
        });
    }
    ...
    function updateSearchResults(results) {
        ...
        var filteredResults = results.filter(function(result) {
            return result.ocAdvisor === "상담사" && result.ocResponseStatus === "답변대기";
        });
